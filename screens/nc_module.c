...
void InitBackground(int argc, char **argv, FILE *fp, int *lc)
{
  ...
  if(n_back_files)
    {
      back_file=fopen(stradp(*(back_files)),"r");
  ...
  while(n_back_files)
      ...
      if(back_file!=(FILE *)NULL)
	{
	  ...
	  for(l=0;(!feof(back_file))&&(l<LINES-2);l++)
		...
		{
		  if(*(back_line+d)=='\t')c+=(8-(c%8));
		  else if(!isspace(*(back_line+d)))
		    (int)mvwaddch(Background_Window,l,c++,*(back_line+d));
		  else c++;
		}
...
static void DisplayBarClock(void)
{
  struct tm *tm;
  char at[20];
  ...
  if(BarClock_l_min!=tm->tm_min)
    {
      ...
      sprintf(at,"%3s %d/%d %d:%2.2d%2s",DOW[tm->tm_wday],tm->tm_mday,tm->tm_mon+1,
  ...
  else touchwin(BarClock_Win);

  wnoutrefresh(BarClock_Win);
  return;
}

static int UpdateBarClock(int r)
{
  struct tm *tm;
...
static void DisplayMailCheck(void)
{
...
static int UpdateMailCheck(int r)
{
  if(r)return 0;
  return(mail_check(1));
}
