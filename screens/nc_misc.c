...
get_args (char *args , int maxlen , char *title)
{
  ...
  fcols=COLS-4;
  if(fcols>72)fcols=72;

  ch= ' ' | A_BOLD;
  if(color==TRUE)ch|=COLOR_PAIR((MENU_EXEC+1));
...
/*  wbkgd(b_win,ch);*/
  ...
  if(color==TRUE)
    {
      ...
      for(r=1;r<4;r++)for(c=1;c<fcols+3;c++)mvwaddch(b_win,r,c,' ');
    }
  ...
  while ((c = wgetch (f_win)) != '\n' && c != ESCAPEKEY && c != 7)
      ...
      if (errno==EIO)
	{
      ...
      else if(c == kill)c=7;   /* Ugg - Hack - see later */

      switch(c)
	{
	case ERR:
	  if(checktty()==0)exit(0);
	  handle_timeout(0);
	  touchwin(b_win); wnoutrefresh(b_win);
	  touchwin(f_win); wnoutrefresh(b_win);
	  doupdate();
	  break;
	...
	case KEY_BACKSPACE:
	  if (x>0)
	    {
	      if(here!=end)memmove(here-1,here,n-x);
	      if((start>0)&&((x==start)||(n<=(start+fcols))))start--;
	      here--,end--,n--,x--;
	      DISPLAY_TEXT;
	    }
	  else
