...
display_items (WINDOW * win, struct menu_items *src, int cols, int hilite,int nocolour)
{
  ...
  else wattrset (win, hilite ? A_REVERSE : A_BOLD);
#endif

  width=strlen(src->name) + (src->type==MENU_SUB?2:0);

  if(align==MIF_NONE) align = (src->type==MENU_TITLE) ? MIF_CENTRE : MIF_LEFT;
  ...
  switch (src->type)
    {
    case MENU_SUB:
      for (i = 0; i < prespace; i++)waddch (win, ' ');
      waddch (win, '<');
  ...
  if (postspace > 0)
    for (i = 0; i < postspace; i++)waddch (win, ' ');
...
void findfirstclickableitem(struct menu_instance *mi)
{
  ...
  for(i=0;i<mi->activepane_lines;i++)
    {
      if((q==NULL)||(ISCLICKABLE(q->type)))break;
      q=q->next;
    }

  if((q==NULL)||(i==mi->activepane_lines))mi->activepane_cursor=mi->activepane_lines-1,mi->mi_highlighted=NULL;
  else mi->activepane_cursor=i,mi->mi_highlighted=q;
}

void findlastclickableitem(struct menu_instance *mi)
{
  struct menu_items *q=mi->mi_activepane_start;
  int i;
  ...
  for(i=0;i<mi->activepane_lines;i++)
    {
      if(ISCLICKABLE(q->type))mi->mi_highlighted=q,mi->activepane_cursor=i;
      q=q->next;
    }
}

static void displaypane(struct menu_instance *mi)
{
  struct menu_items *q=mi->mi_activepane_start;
  chtype ch;
  int i;
  ...
  if(M_SCROLL_BAR)
    {
      ...
      for(n=0;(n<mi->activepane_lines)&&(q!=NULL);n++)q=q->next;
      q=mi->menu->head;
      for(nhead=0;q!=NULL;nhead++)q=q->next;
	...
	wattron (mi->background_win, COLOR_PAIR ((MENU_TITLE + 1)));
      ...
      for(j=0;j<mi->activepane_lines;j++,y++)mvwaddch(mi->background_win,y,mi->activepane_cols+(M_DRAW_BOX?1:0)+1,' ');
  ...
  for(i=0;i<mi->activepane_lines;i++)
    {
      ...
      display_items (mi->activepane_win, q, mi->activepane_cols, 
		     ((q==mi->mi_highlighted)&&((mi->menu->flags & MF_NOCURSOR) == 0)?TRUE:FALSE),
...
static void scrolldown(struct menu_instance *mi,int n)
{
  struct menu_items *q=mi->mi_activepane_start;
  int i,backtrack=1;

  if(n==0)n=mi->activepane_lines,backtrack=0;
  else if(n<0)n=-n,backtrack=0;
...
static void scrollup(struct menu_instance *mi,int n)
{
  struct menu_items *q=mi->mi_activepane_start;
  int i,backtrack=1;
  ...
  if(q==NULL)mi->mi_activepane_start=mi->menu->data,mi->activepane_s=0,mi->activepane_cursor+=(i-1);
  else mi->mi_activepane_start=q,mi->activepane_s-=i,mi->activepane_cursor+=i;
...
void down_hilite (struct menu_instance *mi)
{
  ...
  if(q==NULL)
      ...
      else
	{
	  ...
	  display_items (mi->activepane_win, mi->mi_highlighted, mi->activepane_cols, 
			 FALSE,(mi->menu->flags & MF_NOCOLOUR));
	  ...
	  wmove (mi->activepane_win, mi->activepane_cursor, 0);
	  display_items (mi->activepane_win, mi->mi_highlighted, mi->activepane_cols, 
			 TRUE,(mi->menu->flags & MF_NOCOLOUR));
...
render_instance (struct menu_instance *mi)
{
  ...
  if(M_ISSET(MF_FIXEDSIZE))
    {
      if(mi->menu->ncols>0)mi->background_cols=mi->menu->ncols;
      if(mi->menu->nlines>0)mi->background_lines=mi->menu->nlines;
    }

  if(mi->background_lines>LINES)mi->background_lines=LINES;
  if(mi->background_cols>COLS)mi->background_cols=COLS;

  mi->activepane_lines=mi->background_lines-(nhead+ntail+(nhead>0?1:0)+(ntail>0?1:0)+(M_DRAW_BOX?2:0));
  ...
  if(mi->menu->flags & MF_TOP)menul=(mi->background_lines>=LINES)?0:1;
  else if(mi->menu->flags & MF_BOTTOM)menul=(mi->background_lines>=LINES)?0:LINES-mi->background_lines-1;
  else menul=(LINES - mi->background_lines) / 2;
  
  if(mi->menu->flags & MF_OFFSET)
    {
      menul+=mi->menu->offl;
      if(menul<0)menul=0;
      else if((menul+mi->background_lines)>LINES)menul=LINES-mi->background_lines;
    }

  if(mi->menu->flags & MF_LEFT)menuc=(mi->background_cols>=COLS)?0:1;
  else if(mi->menu->flags & MF_RIGHT)menuc=(mi->background_cols>=COLS)?0:COLS-mi->background_cols-1;
  else menuc=(COLS - mi->background_cols) / 2;
  
  if(mi->menu->flags & MF_OFFSET)
    {
      menuc+=mi->menu->offc;
      if(menuc<0)menuc=0;
      else if((menul+mi->background_cols)>COLS)menuc=COLS-mi->background_cols;
    }

  mi->background_win =
    newwin (mi->background_lines, mi->background_cols,menul,menuc);
...
/*  if((color==TRUE)&&(!nocolour))ch|=COLOR_PAIR((MENU_NOP+1));
  ...
  if(M_DRAW_BOX)box(mi->background_win, 0, 0);
  
  mi->activepane_win =
    derwin(mi->background_win, mi->activepane_lines, mi->activepane_cols,
	   nhead+(nhead>0?1:0)+(M_DRAW_BOX?1:0),(M_DRAW_BOX?1:0));
  ...
  if(nhead>0)
      ...
      while(q)
	{
	  ...
	  display_items (mi->background_win, q, 
			 mi->background_cols-(M_DRAW_BOX?2:0), 
			 FALSE,(mi->menu->flags & MF_NOCOLOUR));
	  q = q->next;
	  i++;
	}
	...
	wattron(mi->background_win,COLOR_PAIR((MENU_NOP+1)));
      ...
      if(M_DRAW_BOX)waddch(mi->background_win,ACS_LTEE);
      wmove(mi->background_win, i, mi->background_cols-1);
      if(M_DRAW_BOX)waddch(mi->background_win,ACS_RTEE);
    }
  ...
  if(M_SCROLL_BAR)
	...
	wattron(mi->background_win,COLOR_PAIR((MENU_NOP+1)));
      ...
      wvline(mi->background_win,ACS_VLINE,mi->activepane_lines+(M_DRAW_BOX?2:((nhead>0?1:0)+(ntail>0?1:0))));
      ...
      if((M_DRAW_BOX)||(ntail>0))
	{
	  ...
	  if((ntail>0)&&(color==TRUE)&&(!nocolour))
	    wattron(mi->background_win,COLOR_PAIR((MENU_NOP+1)));
	  wmove(mi->background_win,mi->background_lines-ntail-(M_DRAW_BOX&&(ntail>0)?1:0)-1,mi->activepane_cols+(M_DRAW_BOX?1:0));
...
void display_screen (int update)
{
  ...
  for(cbl=&DCBs.undermenu,cb=cbl->next;cb!=cbl;cb=cb->next)(*cb->f)();
  ...
  for(cbl=&DCBs.overmenu,cb=cbl->next;cb!=cbl;cb=cb->next)(*cb->f)();
  for(cbl=&DCBs.overall,cb=cbl->next;cb!=cbl;cb=cb->next)(*cb->f)();
...
void handle_timeout (int update)
{
  ...
  for(cbl=&DCBs.timeout,cb=cbl->next;cb!=cbl;cb=cb->next)
    if((*cb->f)(redisp)==1)redisp=1;
  
  if((redisp)&&(update!=-1))display_screen(update);
...
do_menu (struct menu *firstmenu)
{
  ...
  if(find_variable("notimeout",NULL))Timeout=-1;
  ...
  while ((mi_c)&&(mi_c!=mi_ip))
	...
	case 2:  /* Ctrl B */
	  if (mi_c->prev)
	    ...
	    {
	      if(nc_runnable_jobs())display_screen(1);
	      else mi_c = rubout_menu_instance (mi_c);
	    }
...
int has_menu_access(struct menu *menu)
{
  ...
  ch= ' ' | A_BOLD;
  if(color==TRUE)ch|=COLOR_PAIR(PASSWORD_COLOR);
