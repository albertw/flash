...
#define TOP_BAR	     "flash " VERSION " Copyright 1997 Stephen Fegan under the GPL"
#define BOT_BAR	     "A - About flash   ? - List Keys   Ctrl-L - Refresh"
/* #define BOT_BAR	     "A - About flash   ? - List Keys   Ctrl-L - Refresh   Ctrl-G - Display GPL" */
...
typedef enum { UNDERMENU, OVERMENU, OVERALL } DISPLAYCALLBACKWHERE;
...
void render_instance (struct menu_instance *mi);
void nc_lock_screen(void);
void get_password(WINDOW *b_win,WINDOW *win,int Line,int Col,char *area,int maxc,int timeout);
