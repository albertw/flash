...
void task_switch(void)
{
  ...
  ch= ' ';
  if(color==TRUE)ch|=COLOR_PAIR((MENU_TITLE+1));
  ...
  do
    {
      ...
      sprintf(temp,"%d/%d %d:%2.2d%2s  [%d] ",tm->tm_mday,tm->tm_mon+1,
	...
	*(temp+66)='\0';
      ...
      if((j->rst==S_RUN)||(j->rst==S_STOP))wattrset(f_win, A_BOLD);
