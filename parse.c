...
extern struct menu_items **GlobalHotKeys;
...
void set_nobox (int wordc, char **wordv, FILE *fp, int *menuline)
{
  draw_box = 0;
  return;
}

void include_file (int wordc, char **wordv, FILE *fp, int *menuline)
{
  wordc--,wordv++;
  while(wordc--)parsefile(*wordv++);
  return;
}
...
parsefile (char *filename)
{
  ...
  while(line!=NULL)
      ...
      if(wordc)
	{
	  ...
	  if(func)(*func)(wordc,wordv,fp,&menuline);
	  else
	    {
	      char mess[80];
...
void setupglobalhotkeys(void)
{
  ...
  for(i=0;i<256;i++)*(GlobalHotKeys+i)=NULL;

  item=menu->data;
  while(item!=NULL)
    {
      if(item->flags & MIF_HOTKEY)*(GlobalHotKeys+(int)item->hotkey)=item;
      item=item->next;
    }
...
void setuphotkeys(int numberoptions,struct menu_items **HotKeys,struct menu *menu)
{
  ...
  if(GlobalHotKeys!=NULL)
    for(i=0;i<256;i++)*(HotKeys+i)=*(GlobalHotKeys+i);
  
  if(numberoptions)
    {
      int j=1;
      for(item=menu->data;(item)&&(j<=9);item=item->next)
	if((item->type=MENU_SUB)||(item->type=MENU_ARGS)||(item->type=MENU_EXEC)||(item->type=MENU_QUIT)||(item->type=MENU_EXIT)||(item->type=MENU_MODULE))
