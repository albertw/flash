...
enum { RC_RUN, RC_RUN_IF, RC_FIND_ELSE_ENDIF, RC_FIND_ENDIF } rc_mode;
...
void rc_file(int wordc, char **wordv, FILE *fp, int *line_no)
{
  ...
  while(wordc)
    {
      if(strcasecmp(*wordv,"system")==0)rctype=SYSTEM;
      else if(strcasecmp(*wordv,"login")==0)
	{
	  if(!login_shell)return;
	}
      else printf("%d: %s: Unrecognised option %s\n",*line_no,directive,*wordv);
      wordv++,wordc--;
    }
...
void rc_block(int wordc, char **wordv, FILE *fp, int *line_no)
{
  ...
  while(wordc)
    {
      if((strcasecmp(*wordv,"login")==0))processthisblock=login_shell;
      else printf("%d: %s: Unrecognised option %s\n",*line_no,directive,*wordv);
      wordv++,wordc--;
    }
  ...
  while(line)
      ...
      if(strcasecmp(testv,RCBLOCK_END)==0)
	{
	  *line_no=linen;
      ...
      if(processthisblock)processrccommandline(line,rctype,errorprefix);
...
processrcfile (char *rcfilename, enum rcfile_type rctype)
{
  ...
  switch(rctype)
    {
    case SYSTEM:
      rcfilename_full=stradp(rcfilename);
      break;
    case USER:
      pwd=getpwuid(getuid());
      if(pwd!=(struct passwd *)NULL)
	{
	  rcfilename_full=(char *)xmalloc
      ...
      else rcfilename_full=rcfilename;
...
void processrccommandline(char *rcline,enum rcfile_type rctype, char *errorprefix)
{
  ...
  if (parseline (rcline, &argc, argv, MAX_LINE-1) > 0)
    {
      if (argc == 0) /*|| (**argv == COMMENT))*/
	return;
      *(argv + argc) = (char *) NULL;

      for (this = recognised_rc_commands; (this->runrc) != NULL; this++)
	if (strcasecmp ((this->command), *argv) == 0)break;
      ...
      if(this->runrc != NULL)
	{
	  if((rctype==USER)&& 
		...
		case RC_FIND_ELSE_ENDIF:
		  ...
		  if((strcmp(*argv,RC_ELSE)==0)||
		     (strcmp(*argv,RC_ENDIF)==0)||
		     (strcmp(*argv,RC_IF)==0))
		    (this->runrc) (argc, argv);
		  break;
	...
	{
#ifdef ALLOW_POO
	  if(strcmp(*argv,"poos")==0)rc_arseme(argc,argv);
	  else 
#endif
...
void rc_set(int argc, char **argv)
{
  ...
  if(strcasecmp(*argv,"set-f")==0)resettable=0;
...
/*      if(**argv==COMMENT)break;
      ...
      else*/ if((strcmp(*argv,"except")==0)&&(!except))except=1;
      ...
      else 
	{
	  for (this = recognised_rc_commands; (this->runrc) != NULL; this++)
	    if (strcmp ((this->command), *argv) == 0)
	      {
		if(except)this->flags&=(!rcflag_restricted);
		else this->flags|=rcflag_restricted;
		break;
	      }
...
rc_if(int argc, char **argv)
{
  ...
  if((argc!=0)&&(strcmp(*argv,"!")==0))not=1,argc--,argv++;

  if(argc!=0)
    {
      success=do_exec((char *)NULL,argc,argv,EXEC_NEVERPAUSE | EXEC_NONBGPROC);
      if(((not==0)&&(success==EXIT_SUCCESS))||
	 ((not==1)&&(success!=EXIT_SUCCESS)))success=1;
      else success=0;
    }

  if(success==1)rc_mode=RC_RUN_IF;
  else rc_mode=RC_FIND_ELSE_ENDIF;
...
rc_arseme(int argc, char **argv)
{
  ...
  while((argc)&&(strcmp(*argv,"poos")==0))arses++,argc--,argv++;
