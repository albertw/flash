...
exec_item (struct menu_items *item)
{
  ...
  switch (item->type)
    {
    case MENU_ARGS:
      if (get_args (args, 1024 , item->prompt) == 0)
	{
	  if((*item->args != '\0')
	     &&(*(item->args + strlen(item->args) -1) == '/'))
	    {
	      char *l=args;
	      while((*l!='\0')&&(*l!='/')&&(!isspace(*l)))l++;
	      if(*l=='/')
		{
	      ...
	      else sprintf (exec, "%s%s", item->args, args);
	    }
	  else sprintf (exec, "%s %s", item->args, args);
...
void finish_jobq(void)
{
  ...
  for(j=JobHead->next;j!=JobHead;j=jn)
    {
      ...
      fprintf(stdout,"%d/%d %d:%2.2d%2s  [%d] %s  (%s)\n",tm->tm_mday,tm->tm_mon+1,
	      ((tm->tm_hour+11)%12)+1, tm->tm_min, tm->tm_hour>11?"pm":"am",
	      j->pgrp,j->cmdline,job_status(j));
  ...
  if(foundrunning==1)
	...
	{
	  jn=j->next;
	  if((j->rst==S_RUN)||(j->rst==S_STOP))kill_job(j);
	  remove_job(j);
	}
...
struct job_q *add_job(char *cmd, pid_t pgrp, pid_t waiton, int eflags, struct termios *ttystate)
{
  struct job_q *j;
  char *seek;
  ...
  else j->ttystate=j->origstate=NULL;

  *(j->cmdline+60)='\0';

  if(cmd==NULL)strcpy(j->cmdline,"No Name");
  else
    {
      while((*cmd==' ')||(*cmd=='\t'))cmd++;
      seek=strchr(cmd,' ');
      if(seek==NULL)seek=cmd;
      else while((seek>cmd)&&(*seek!='/'))seek--;
      if(*seek=='/')seek++;
...
void remove_job(struct job_q *j)
{
  ...
  free(j->cmdline);
  if(j->ttystate)free(j->ttystate);
  if(j->origstate)free(j->origstate);
  free(j);
...
void promote_job(struct job_q *j)
{
  j->prev->next=j->next;
  j->next->prev=j->prev;
...
int wait_fg(struct job_q *j, sigset_t *mask)
{
  ...
  if(j->origstate!=NULL)tcsetattr(0,TCSADRAIN,j->origstate);
  ...
  switch(j->rst)
    ...
    case S_KILL:
      ...
      if((j->eflags & EXEC_NEVERPAUSE)==0)pressanykey();
      ret=-1;
      remove_job(j);
      break;
    ...
    case S_EXIT:
      ...
      {
	if((j->eflags & EXEC_NOPAUSE)==0)pressanykey();
	else if(j->exit != EXIT_SUCCESS)pressanykey();
      }
    ...
    case S_STOP:
      if(j->signal==SIGTSTP)fprintf(stderr,"Process Stopped by user ");
      else fprintf(stderr,"Process Stopped by signal ");
      if((j->eflags & EXEC_NEVERPAUSE)==0)pressanykey();
      ret=-1;
      break;
...
int runnable_jobs(void)
{
  ...
  while(j!=JobHead)
    {
      if((j->rst==S_RUN)||(j->rst==S_STOP))break;
      j=j->prev;
    }

  if(j!=JobHead)return 1;
  else return 0;
}
...
int run_in_fg(struct job_q *j)
{
  ...
  if(j->ttystate!=NULL)tcsetattr(0,TCSADRAIN,j->ttystate);
...
{
  if(j->jflags & JOBF_HUPSENT)kill(-j->pgrp,SIGKILL);
  else 
    {
      kill(-j->pgrp,SIGHUP);
      run_in_bg(j);
...
static void exec_prog(char **argv)
{
  int n;
  char *path;
...
int do_exec (char *exec_line, int argc, char **argv, int eflags)
{
  ...
  if (page)
    {
      if (pagerv == NULL)setup_pager();
      if ((pagerv == NULL) || (pipe (pipefd) < 0))page = 0;
    }
  ...
  if(tcgetattr(0,&Ttystate)!=-1)ttystate=&Ttystate;
  else ttystate=NULL;
  ...
  if ((pid_child = fork ()) == -1)
      ...
      if (page)
	{
	  ...
	  if (pipefd[1] != 1)
	    {
	      if (dup2 (pipefd[1], 1) != 1)error ("Could not dup pipe fd");
	      else close (pipefd[1]);
	    }
	}
	...
	{
	  if(!page)syslog (LOG_LOCAL1 | LOG_INFO, "%d: %s (%d)", 
			   getpid (), exec_line, pid_child);
	  else syslog (LOG_LOCAL1 | LOG_INFO, "%d: %s (paged) (%d)", 
		       getpid (), exec_line, pid_child);
	}
      ...
      if (page)
	{
	  ...
	  if (pid_pager == -1)
	  ...
	  else if (pid_pager == 0)
	    {
	      ...
	      if (pipefd[0] != 0)
		{
		  if (dup2(pipefd[0], 0) != 0)error ("Could not dup pipe fd");
		  else close (pipefd[0]);
		}
      ...
      if((eflags & EXEC_BACKGROUND)==0)
	{
	  ...
	  while(((eflags & EXEC_NONBGPROC) != 0) && (exitstat==-1) && 
		(j->rst == S_STOP))
	    {
      ...
      else if((eflags & (EXEC_NEVERPAUSE|EXEC_NOPAUSE))==0)
	{
	  sleep(1);
...
handle_children (void)
{
  ...
  while ((pid = waitpid (-1, &status, WNOHANG | WUNTRACED)) > 0)
	...
	if((pid==x->waiton)||(pid==x->pgrp))break;

      if(x==JobHead)continue;
  ...
  if(canjump)siglongjmp(jmpbuf,1);
  return;
}
