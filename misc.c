...
int mail_check(int check_type)
{
  ...
  if((at>=mt)||(mt<start_time))retval=0;
  else
    {
      if((mt>mail_notify_time)&&(was_there_mail_last_time==0))retval=2;
      else retval=1;

      if(check_type==0)time(&mail_notify_time);
    }
...
int setenv(const char *name, const char *value, int overwrite)
{
  ...
  x=environ;
  n=0;
  while((*x!=NULL)&&((memcmp(*x,name,nlen)!=0)||(*(*x+nlen)!='=')))x++,n++;
  n++;
...
#ifdef DEBUG
  else fprintf(stderr,"setenv: NOT setting variable %s\n",name);
#endif
...
void unsetenv(const char *name)
{
  ...
  x=environ;
  while((*x!=NULL)&&((memcmp(*x,name,nlen)!=0)||(*(*x+nlen)!='=')))x++;
  if(*x!=NULL)
    {
#ifdef DEBUG
...
#ifdef DEBUG
  else fprintf(stderr,"unsetenv: Could not find variable %s\n",name);
#endif
