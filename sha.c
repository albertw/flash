...
char **argv;
{
  ...
  if(argc>1)
    {
      if(strlen(*(argv+1))!=3)printf("Salt must be 3 characters\n");
      else printf("%s\n",shacrypt(*argv,*(argv+1)));
    }
  else printf("Usage: %s key salt\n",prog);
}
...
char *shacrypt(char *key,char *salt)
{
  ...
  for(i=0;i<16;i++)
    {
      ...
      sha_memory(hhbuf,sizeof(hbuf),hbuf);
#ifdef LITTLE_ENDIAN
      byteReverse(hbuf, 4*HASH_SIZE );
#endif /* LITTLE_ENDIAN */
    }
  ...
  for(i=0;i<sizeof(hbuf);i++,t++,f++)*t=bin_to_ascii((*f)&0x3f);
  *t='\0';
