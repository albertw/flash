...
static void initialise_menu(struct menu *m)
{
  m->name=NULL;
  m->password=NULL;
  m->flags=MF_NONE;
...
static int Menu_name_compare(void *a,void *b)
{
  return strcmp(((struct menu *)a)->name,((struct menu *)b)->name);
}

void add_menu(struct menu *m,int line)
{
  if(Menus==NULL)Menus=new_dict(Menu_name_compare);
...
struct menu *find_menu (char *name)
{
  struct menu k;
  k.name=name;
  
  if(Menus!=NULL)return (struct menu *)find_node(Menus,&k);
  else return NULL;
}
...
void set_mainmenu(int wordc, char **wordv, FILE *fp, int *menuline)
{
  if(wordc<2)fprintf(stderr,"%s: %s: Must specify name\n",progname,*wordv);
  else 
    {
...
void freemenu(struct menu *m)
{
  ...
  item=m->data;
  while((item) && (item->prev))item=item->prev;
  ...
  while(item!=NULL)
    {
      ...
      if(item->name)free(item->args);
      if(item->name)free(item->prompt);
#endif
      free(item);
      item=m->data;
    }
...
alloc_items (char *line, int i)
{
  ...
  if((ntokens) && (flags != M_NONE))
    {
      ...
      if((flags & M_ARGS) == 0)
	{
	  if(flags & M_FLAGS)flags_p=tokens[0];
	}
      else
	{
	  /* This is the number of options that we want at most */
	  maxoptions= 1 /* M_ARGS */ + (flags & M_PROMPT) + (flags & M_FLAGS);
	  
	  if((ntokens==1)||(maxoptions==1))args_p=tokens[0];
	  else if((ntokens==2)||(maxoptions==2))
	    {
	      if(flags & M_PROMPT)args_p=tokens[0],prompt_p=tokens[1];
	      else if(flags & M_FLAGS)args_p=tokens[1],flags_p=tokens[0];
	      else error("Warning: Boolean Logic failure"); /* This should not happen */
	    }
	  else /* if(ntokens>=3) */
	    {
	      args_p=tokens[1];
  ...
  else if(flags & M_ARGS)parse_error("Premature end-of-line\n", i);
...
static void process_menu_items(struct menu *menu, FILE *fp, int *start_line)
{
  ...
  while(line)
      ...
      else
	{
	  ...
	  if((d->flags&MIF_HEAD)!=0)attach_here=&(menu->head);
	  else if((d->flags&MIF_TAIL)!=0)attach_here=&(menu->tail);
	  else attach_here=&(menu->data);

	  if(*attach_here==NULL)*attach_here=d;
	  else
	    {
	      d->prev=*attach_here;
...
void define_menu (int wordc, char **wordv, FILE *fp, int *i)
{
  struct menu *menu;
  ...
  while(wordc)
    {
      if(strcasecmp(*wordv,MOPT_NOBOX)==0)menu->flags |= MF_NOBOX;
      else if(strcasecmp(*wordv,MOPT_LEFT)==0)menu->flags = (menu->flags & ~MF_HMASK) | MF_LEFT;
      else if(strcasecmp(*wordv,MOPT_RIGHT)==0)menu->flags = (menu->flags & ~MF_HMASK) | MF_RIGHT;
      else if(strcasecmp(*wordv,MOPT_TOP)==0)menu->flags = (menu->flags & ~MF_VMASK) | MF_TOP;
      else if(strcasecmp(*wordv,MOPT_BOTTOM)==0)menu->flags = (menu->flags & ~MF_VMASK) | MF_BOTTOM;
      else if(strcasecmp(*wordv,MOPT_OFFSET)==0)
	{
	  if(wordc>=3)
	    {
      ...
      else if(strcasecmp(*wordv,MOPT_SIZE)==0)
	{
	  if(wordc>=3)
	    {
      ...
      else if(strcasecmp(*wordv,MOPT_PASSWORD)==0)
	{
	  if(wordc>=2)
	    {
      ...
      else if(strcasecmp(*wordv,MOPT_NOCOLOUR)==0)menu->flags |= MF_NOCOLOUR;
      else if(strcasecmp(*wordv,MOPT_SCROLLBAR)==0)menu->flags |= MF_SCROLLBAR;
      else if(strcasecmp(*wordv,MOPT_NOCURSOR)==0)menu->flags |= MF_NOCURSOR;
      else if(strcasecmp(*wordv,MOPT_OVERLAYMENU)==0)menu->flags |= MF_OVERLAY;
